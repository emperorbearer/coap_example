...
기존 벽스위치 뒤에 들어가는 "이너릴레이" 형태의 **배터리 구동 무선 스위치**가,
Thread 메시 위에서 **CoAP 바인딩**으로 전등을 직접 켜고 끕니다.
전등은 상시 전원으로 동작하며, **CoAP→MQTT 브리지**를 통해 Home Assistant 같은
스마트홈 플랫폼에 상태를 노출합니다.
...
| `docs/hardware/switch-node.md` | 스위치 노드(nRF54L15) 하드웨어 설계·폼팩터·BOM |
| `docs/hardware/light-node.md` | 전등 노드(멀티 MCU) 하드웨어 설계·BOM |
| `docs/home-assistant.md` | CoAP→MQTT 브리지 및 Home Assistant 연동 설계 |
| `firmware/common/` | 두 노드가 공유하는 CoAP 자원/바인딩 정의 |
| `firmware/switch/` | 스위치 노드 Zephyr 앱 (nRF54L15) |
| `firmware/light/` | 전등 노드 Zephyr 앱 (멀티 보드) |
| `bridge/` | CoAP→MQTT 브리지 (Python / aiocoap) |

## 대상 플랫폼

- **스위치 노드**: nRF54L15 — 초저전력, Thread Sleepy End Device(SED)로 배터리 수명 극대화.
- **전등 노드**: Zephyr가 Thread를 지원하는 다음 MCU를 목표로 하며 보드 오버레이로 이식.
  - nRF54L15 / nRF52840 (Nordic)
  - MGM240 / EFR32MG24 (Silicon Labs)
  - ESP32-H2, ESP32-C6 (Espressif)
