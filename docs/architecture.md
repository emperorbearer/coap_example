...
배터리 수명이 이 프로젝트의 핵심 제약이다. 라디오 절전 전략은 두 스위치가 공유하되,
**전원(배터리) 구성은 타입별로 다르다.**

| 타입 | 배터리 | 전원 관리 | 충전 |
|------|--------|-----------|------|
| 이너릴레이 스위치 | 원형 소형 1차 전지(코인셀 CR2032/2450) | PMIC 없이 SoC 내장 DC/DC 직결 | 교체식(충전 없음) |
| 월스위치(패널) | 리튬이온 충전지(보호셀) | **nPM1300 PMIC**(충전 + 2×벅/LDO + 퓨얼게이지, I2C) | **USB-C** 재충전 |

- **라디오 정책**: 둘 다 SSED. 슬립 중 라디오 오프, CSL period 를 수백 ms~수 초로
  설정해 평균 전류를 최소화.
- **입력 감지**: 접점/버튼/엔코더를 GPIO SENSE(래치) 로 감시. 상태 변화 시에만 CPU wake.
  풀업/풀다운은 누설 최소화를 위해 이벤트 시에만 능동화하거나 큰 저항을 사용.
- **송신 예산**: 조작 1회당 CoAP 왕복(≈수십 ms 라디오 on). 평균 소비는 슬립 전류가 지배.
  코인셀(CR2450)은 펄스전류에 취약하므로 보조 커패시터로 TX 순간 전류를 보조한다.
- **PMIC 채택 이유(패널)**: 충전식 리튬이온은 충전기+레귤레이터가 필요한데, nPM1300 은
  USB-C 충전기·다중 레일·연료게이지 측정을 통합하고 저전력 모드(Ship/Hibernate)로
  SSED 대기 소비를 해치지 않는다. 잔량은 nPM1300 측정값 + nRF Fuel Gauge 라이브러리로
  산출하고, I2C 로 nRF54L15 가 제어한다.
- **자세한 계산·전원 회로**: `docs/hardware/switch-node.md`(코인셀),
  `docs/hardware/panel-switch.md`(리튬이온+nPM1100) 참고.
