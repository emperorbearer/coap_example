...
아니라 CW/WW 화이트 LED 를 PWM 으로 직접 구동해 **색온도(CCT)·디밍**을 제공한다(튜너블 화이트).
...
- **라디오 정책**: 둘 다 SSED이며 **발신 전용**이라 CSL 수신을 끈다. 슬립 중 라디오
  오프, 부모와의 링크는 장주기 데이터폴(킵얼라이브)로만 유지 → 평균 전류 최소화.
...
- **조작 횟수의 영향**: 하루 수십 회 조작은 평균 전류에 거의 영향이 없다(≈µA 이하).
  수명은 유휴 전류(CSL/폴·누설)가 결정한다.
- **자세한 계산·전원 회로**: `docs/hardware/power-budget.md`(수명 분석),
  `docs/hardware/switch-node.md`(코인셀), `docs/hardware/panel-switch.md`(리튬이온+nPM1300).
