...
| 월스위치(패널) | 리튬이온 충전지(보호셀) | **nPM1100 PMIC**(충전 + 벅, 저Iq) | USB-C/충전접점 재충전 |
...
- **PMIC 채택 이유(패널)**: 충전식 리튬이온은 충전기+레귤레이터가 필요한데, nPM1100 은
  이 둘을 통합하고 정지전류가 낮아 SSED 대기 소비를 해치지 않는다. 퓨얼게이지가 없어
  잔량은 MCU ADC 로 VBAT 을 측정해 추정(필요 시 nPM1300 로 상향).
