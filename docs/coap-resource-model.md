...
상태 객체 스키마(논리 구조, JSON 표기):

```json
{
  "on":    true,          // 필수. 전원 on/off
  "bri":   254,           // 선택. 마스터 밝기 1..254 (0 은 사용 안 함), 미지원 시 생략
  "r":     255,           // 선택. 적색 0..255 (RGB 지원 시)
  "g":     120,           // 선택. 녹색 0..255
  "b":     0,             // 선택. 청색 0..255
  "w":     0,             // 선택. 백색 채널 0..255 (RGBW 등기구)
  "ct":    370,           // 선택. 색온도 mireds (튜너블 화이트 지원 시)
...
|------|------|------|
| 디밍 | `bri` | 마스터 밝기 |
| RGB 색상 | `r`,`g`,`b` | 커스텀 LED PCB(§전등 HW) |
| 백색 채널 | `w` | RGBW 등기구 |
| 색온도 | `ct` | 튜너블 화이트(mireds) |

**부분 갱신 규칙**: `PUT` 는 포함된 필드만 적용한다. 색상 변경은 자동으로 `on:true` 를
함의한다(꺼진 상태에서 색만 바꾸면 켜짐). `bri` 는 색과 독립적인 마스터 밝기이며,
드라이버는 최종 채널 출력을 `channel * bri / 254` 로 스케일한다.
